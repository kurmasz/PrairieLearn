...
module.exports.getGroupInfo = function (assessmentId, userId, callback) {
  ...
  sqldb.query(sql.get_config_info, params, function (err, result) {
    ...
    sqldb.query(query, params, async function (err, result) {
      ...
      // As far as I can tell, isGroupMember and groupsize === 0 are the same?
      ...
      // This variable is used in the case that the user tried and failed to join a group.
      const used_join_code = groupsize >= maxsize ? true : undefined; // TODO: Improve!
      let start;
      let join_code
      ...
      if (isGroupMember) {
        ...
        if (usingGroupRoles) {
          ...
          const result = await sqldb.callAsync('group_info_roles_validate', params);
...
module.exports.updateGroupRoles = function (
  ...
  sqldb.call('group_roles_validate', params, function (err, result) {
    if (ERR(err, callback)) return;
